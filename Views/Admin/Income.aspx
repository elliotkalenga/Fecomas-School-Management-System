<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Income.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Income" %>
...
<div class="card-header d-flex justify-content-between align-items-center p-2">
    ...
    <div class="d-flex flex-wrap gap-2">
        <div class="input-group" style="height: 30px;">
            <input type="text" id="searchInput" class="form-control" placeholder="Search for Income Trans...">
    ...
    <i class="fas fa-book-medical"></i> Add New Income
</button>
</th>
                            </tr>
                        </thead>
        ...
        onclick="openEditModal3('<%# Eval("Term") %>', '<%# Eval("IncomeId") %>')">
    <i class="fas fa-print"></i> 
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("IncomeId") %>')">
                                                <i class="fas fa-edit"></i> 
                                            </button>
                                            <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("IncomeId") %>')">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="studentModalLabel3">Financial Reports</h5>
                    <button type="button" class="close" data-dismiss="modal" onclick="refreshPage()" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <iframe id="studentIframe3" src="IncomeStatement.aspx" frameborder="0"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="studentModal" tabindex="-1" role="dialog" aria-labelledby="studentModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="studentModalLabel">Income Transactions Management</h5>
                    <button type="button" class="close" data-dismiss="modal" onclick="refreshPage()" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <iframe id="studentIframe" src="IncomeAdd.aspx" frameborder="0"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(IncomeId) {
            document.getElementById('studentIframe').src = 'IncomeAdd.aspx?IncomeId=' + IncomeId;
        }



        function openEditModal3(term, requisitionId) {
            document.getElementById('studentIframe3').src = 'IncomeStatement.aspx?Term=' + encodeURIComponent(term) + '&IncomeId=' + encodeURIComponent(requisitionId);
        }
        ...
        function confirmDelete(IncomeId) {
            if (confirm("Are you sure you want to delete this Book?")) {
                window.location.href = 'IncomeAdd.aspx?IncomeId=' + IncomeId + '&mode=delete';
            }
        }
