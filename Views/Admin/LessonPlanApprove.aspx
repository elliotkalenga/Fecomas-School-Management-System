<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="LessonPlanApprove.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.LessonPlanApprove" %>
...
<div class="card-header d-flex justify-content-between align-items-center p-2">
    <div class="d-flex flex-wrap align-items-center gap-2">
        <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
            <i class="fas fa-plus"></i> Add New Lesson Plan
        </button>
    </div>

        <div class="d-flex flex-wrap align-items-center gap-2">
        <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#reportModal">
            <i class="fas fa-file-art"></i> Generate Lesson Plans Reports
        </button>
    </div>

    <div class="d-flex flex-wrap gap-2">
        <input type="text" id="searchInput" class="form-control" placeholder="Search for Lesson Plan..." style="height: 30px;" onkeyup="searchTable()">
    </div>
</div>
        <div class="card-body">
                ...
                <table id="studentsTable" class="table table-striped table-bordered" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>Lesson Outcomes</th>
                            <th>Introduction</th>
                            <th>Planned Activities</th>
                            <th>Resources</th>
                            <th>Assessment Criteria</th>
                            <th>Lesson Evaluation</th>
                            <th>Teacher</th>
                        ...
                        <asp:Repeater ID="RecordsRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    <td><%# Eval("LessonTopic") %></td>
<td><%# Regex.Replace(Eval("Schemesofwork").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("LessonObjectives").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("LessonOutcome").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("Introduction").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("PlannedActivities").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("Resources").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("AssessmentCriteria").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("LessonEvaluation").ToString(), @"[\n.]+", "<br>") %></td>

                                    <td><%# Eval("CreatedBy") %></td>
<td>
    <%# "<span style='color: " + 
        (Eval("CheckStatus").ToString() == "Checked" ? "#008000" : "#FF0000") + "'>" + 
        "<i class='fa " + 
        (Eval("CheckStatus").ToString() == "Checked" ? "fa-check-circle" : "fa-times-circle") + "'></i> " + 
        Eval("CheckStatus") + 
        "</span>" %>
</td>                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("LessonId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button></td><td>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("LessonId") %>')">
    ...
    <div class="modal-dialog modal-lg" role="document" style="max-width: 90vw; width: 90vw; height: 90vh;">
        <div class="modal-content" style="height: 100%;">
            <div class="modal-header">
                <h5 class="modal-title" id="reportModalLabel">Lesson Plans Reports</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" style="height: calc(100% - 50px); padding: 0;">
                <iframe id="reportIframe" src="LessonReport.aspx" frameborder="0" style="width: 100%; height: 100%; border: none;"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="studentModal" tabindex="-1" role="dialog" aria-labelledby="studentModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Lesson Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="LessonPlansAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="successModal" tabindex="-1" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to delete this Lesson Plan Record?")) {
                window.location.href = 'LessonPlansAdd.aspx?LessonId=' + LessonId + '&mode=delete';
            }
        }

        function openEditModal(LessonId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'LessonPlansAdd.aspx?LessonId=' + LessonId;
        }
        ...
        $(document).ready(function () {
            ...
            if (urlParams.get('success') === 'true') {
                ...
                $('#successModal').modal('show');
                // Remove the query parameter after displaying the message
                history.replaceState(null, '', window.location.pathname);
            }
        });
        $(document).ready(function () {
