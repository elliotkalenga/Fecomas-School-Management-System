<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="PeriodRegister.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.PeriodRegister" %>
...
<div class="card-header d-flex justify-content-between align-items-center p-2">
    <div class="d-flex flex-wrap align-items-center gap-2">
        <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
            <i class="fas fa-plus"></i> Add New Period Register Record
        </button>
    </div>
            <div class="d-flex flex-wrap align-items-center gap-2">
        <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#reportModal">
            <i class="fas fa-file-art"></i> Generate Reriod Register Reports
        </button>
    </div>
...
<td><%# Regex.Replace(Eval("LessonPlan").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("ClassStream").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("Fromtime").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("Totime").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Eval("duration") %></td>
<td><%# Regex.Replace(Eval("Attendance").ToString(), @"[\n.]+", "<br>") %></td>
<td><%# Regex.Replace(Eval("Teacher").ToString(), @"[\n.]+", "<br>") %></td>

                                    <td><%# Eval("CreatedBy") %></td>
<td>                                        
                                    <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("RegisterId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button></td><td>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("RegisterId") %>')">
...
<div class="modal fade" id="studentModal" tabindex="-1" role="dialog" aria-labelledby="studentModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Period Register Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="PeriodRegisterAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="successModal" tabindex="-1" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog2" role="document" style="max-width: 400px;">
        <div class="modal-content">
            ...
            <div class="modal-body text-success" style="border: 2px solid green;">
               Period Register deleted successfully!
            </div>
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            function confirmDelete(RegisterId) {
            if (confirm("Are you sure you want to delete this Period Register Record?")) {
                window.location.href = 'PeriodRegister.aspx?RegisterId=' + RegisterId + '&mode=delete';
            }
        }

        function openEditModal(RegisterId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'PeriodRegisterAdd.aspx?RegisterId=' + RegisterId;
        }
