...
{
    public partial class FeesCollectionSUmmaryAdd : System.Web.UI.Page
    {
...
WHERE (Term = @Term) AND (SchoolCode = @SchoolCode)";
        ...
        protected void btnResultsheet_Click(object sender, EventArgs e)
        {
            ...
            // Define the query
            string query = @"SELECT feescollectionId, SchoolName, SchoolCode, Logo, CreatedBy, CreatedDate, Address, ReferenceNo, Student, StudentNo, InvoiceId, PaymentMethod, Fees, ReceiptAmount, Cum_Collection, balance, InvoiceStatus, Term,ClassName
FROM   FeesCollectionTransactions
WHERE (SchoolCode = @SchoolCode) AND (Term = @Term) Order By ClassName,InvoiceStatus";
            ...
            else
            {
                ...
                // Set the ReportViewer properties
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/FeesCollectionTransactions.rdlc");

                // Enable external images
                ReportViewer1.LocalReport.EnableExternalImages = true;
                // Add the data source to the report
                ReportDataSource reportDataSource = new ReportDataSource("FeesCollectionTransactions", dataTable);
                ReportViewer1.LocalReport.DataSources.Clear();
                ReportViewer1.LocalReport.DataSources.Add(reportDataSource);
                // Set the external images path
                // string imagePath = "file:///C:/Logo/";
        ...
        protected void btnInvoicesStatus_Click(object sender, EventArgs e)
        {
            // Define the query
            string query = @"SELECT EnrollmentID, Phone, SchoolId, Paidstatus, Guardian, InvoiceId, InvoiceNo, Student, ClassName, StudentNO, TotalCollected, Balance, Term, Status, CreatedDate, CreatedBy, SchoolName, SchoolCode, Logo, Logoid, Address, TermId, InvoiceStatus, Fees
FROM   InvoiceStatus
WHERE (Term = @Term) AND (SchoolCode = @SchoolCode) Order By ClassName,InvoiceStatus,Student";
            ...
            else
            {
                ...
                // Set the ReportViewer properties
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/InvoiceStatus.rdlc");

                // Enable external images
                ReportViewer1.LocalReport.EnableExternalImages = true;
                // Add the data source to the report
                ReportDataSource reportDataSource = new ReportDataSource("InvoiceStatus", dataTable);
                ReportViewer1.LocalReport.DataSources.Clear();
                ReportViewer1.LocalReport.DataSources.Add(reportDataSource);
                // Set the external images path
                // string imagePath = "file:///C:/Logo/";
        ...
        protected void btnCollectionSummary_Click(object sender, EventArgs e)
        {
            ReportViewer1.LocalReport.DataSources.Clear();
            ReportViewer1.LocalReport.Refresh();
