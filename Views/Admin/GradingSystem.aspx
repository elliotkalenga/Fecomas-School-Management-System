<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="GradingSystem.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.GradingSystem" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Grading Scale
                </button>
            ...
            <div class="d-flex flex-wrap gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search for Grading Scale..." style="height: 30px;">
                        ...
                        <asp:Repeater ID="RecordRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    <td><%# Eval("LowerSCale") %></td>
                                    <td><%# Eval("UpperScale") %></td>
                                    <td><%# Eval("Grade") %></td>
                                    <td><%# Eval("GradeDescription") %></td>
                                    <td><%# Eval("Remark") %></td>
                                    <td><%# Eval("GradingSystem") %></td>
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("ScaleId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("ScaleId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Grading System Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="GradingSystemAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        function confirmDelete(ScaleId) {
            if (confirm("Are you sure you want to delete this Term?")) {
                window.location.href = 'GradingSystemAdd.aspx?ScaleId=' + ScaleId + '&mode=delete';
            }
        }

        function openEditModal(ScaleId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'GradingSystemAdd.aspx?ScaleId=' + ScaleId;
        }
