<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="BorrowBook.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.BorrowBook" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Borrow a Book from Library
                </button>
                <button type="button" class="btn btn-success mb-1" onclick="exportTableToCSV('BooksBorrowed.csv')">
                    <i class="fas fa-file-csv"></i> Export CSV
                </button>
                <button type="button" class="btn btn-danger mb-1" onclick="exportTableToPDF('BooksBorrowed.pdf')">
                    <i class="fas fa-file-pdf"></i> Export PDF
                </button>
            </div>
            <div class="d-flex flex-wrap gap-2">
<input type="text" id="searchInput" class="form-control" placeholder="Search for Books..." style="height: 30px;" onkeyup="searchTable()">
                ...
                <asp:Repeater ID="RecordRepeater" runat="server">
                    <ItemTemplate>
                        <tr class="student-row">
                            ...
                            <td><%# Eval("BorrowedDateString") %></td>
                            <td><%# Eval("DateReturnString") %></td>
                            <td><%# Eval("ActualReturnDateString") %></td>

                            <!-- Conditional ForeColor for BookStatus with Icon and Text -->
                            <td>
                                <%# Eval("BookStatus").ToString() == "Returned" ? 
                                    "<i class='fas fa-thumbs-up text-success'></i> Returned" : 
                                    "<i class='fas fa-times text-danger'></i> Not Returned" 
                                %>
                            </td>

                            <td>
<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("BorrowingId") %>')">
    <i class="fas fa-book"></i> <i class="fas fa-undo-alt"></i> Return
</button>
                            </td>
                        </tr>
                    </ItemTemplate>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Borrow/Return Books Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="BorrowBookAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="successModal" tabindex="-1" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        $(document).ready(function () {
            // Refresh the page when the studentModal is closed
            $('#studentModal').on('hidden.bs.modal', function () {
                location.reload();
            });

            // Prevent page reload for the successModal
            $('#successModal').on('hidden.bs.modal', function () {
                // Add any custom behavior here if necessary
            });
        });
        ...
        function confirmDelete(BorrowingId) {
            if (confirm("Are you sure you want to delete this Term?")) {
                window.location.href = 'BorrowBookAdd.aspx?BorrowingId=' + BorrowingId + '&mode=delete';
            }
        }

        function openEditModal(BorrowingId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'BorrowBookAdd.aspx?BorrowingId=' + BorrowingId;
        }
        ...
        function searchTable() {
            ...
            var tr = document.getElementsByClassName("student-row");
            var visibleRows = [];
            ...
            for (var i = 0; i < tr.length; i++) {
                ...
                if (match) visibleRows.push(tr[i]);
            }

            document.getElementById("noDataRow").style.display = visibleRows.length === 0 ? "" : "none";

            // Reset and rebuild pagination
            var pageCount = Math.ceil(visibleRows.length / rowsPerPage);
            ...
            $('.page-link').click(function (e) {
                e.preventDefault();
                var page = $(this).data('page');
                showPage(visibleRows, page);
            });

            showPage(visibleRows, 1); // Show first page of filtered results
        }
