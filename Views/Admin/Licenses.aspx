<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Licenses.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Licenses" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New License
                </button>
            ...
            <div class="d-flex flex-wrap gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search for Class..." style="height: 30px;">
                        ...
                        <asp:Repeater ID="RecordRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    ...
                                    <td><%# Eval("LicensedDays") %></td>
                                    <td><%# Eval("UsedDays") %></td>
                                    <td><%# Eval("RemainingDays") %></td>
                                    <td><%# Eval("LicenseStatus") %></td>
                                    <td><%# Eval("ExpiryDateString") %></td>
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("LicenseId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("LicenseId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">License Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="LIcenseAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        function confirmDelete(SchoolId) {
            if (confirm("Are you sure you want to delete this License?")) {
                window.location.href = 'LicenseAdd.aspx?LicenseId=' + SchoolId + '&mode=delete';
            }
        }

        function openEditModal(SchoolId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'LicenseAdd.aspx?LicenseId=' + SchoolId;
        }
