<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/Master.Master" AutoEventWireup="true" CodeBehind="ExpenseItemAdd.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.ExpenseItemAdd" %>
...
<div class="row">
    <div class="form-group col-md-6">
        <label for="txtItem">Expense Item</label>
        <div class="input-group">
            ...
            <asp:TextBox ID="txtItem" runat="server" CssClass="form-control" 
                 placeholder="Enter Expense Item"></asp:TextBox>
        </div>     </div>
    <div class="form-group col-md-5">
        ...
        <div class="input-group">
            ...
            <asp:TextBox ID="txtAmount" runat="server" CssClass="form-control" 
                 placeholder="Enter Expense Amount"></asp:TextBox>
        </div>     </div>
 

    <div class="form-group col-md-1">
        <label for="txtExpenseId"></label>
        <div class="input-group">
            <div class="input-group-prepend">
            </div>
            <asp:TextBox ID="txtExpenseId"  ReadOnly="true"  required runat="server" CssClass="form-control" placeholder="Enter Requisition ID"></asp:TextBox>
        </div>
    </div>
    ...
    <i class="fas fa-book-medical"></i>Action
                        ...
                        <tbody>
                            <asp:Repeater ID="RecordsRepeater" runat="server">
                                <ItemTemplate>
                                    <tr>
                                        <td><%# Eval("ExpenseItemId") %></td>
                                        <td><%# Eval("ExpenseItemName") %></td>
                                    <td><%# "MK " + string.Format("{0:N2}", Eval("Amount")) %></td>
                                        <td>
    <span style='color: <%# Eval("Notes").ToString() == "Pending" ? "chocolate" : "darkgreen" %>;'>
        <i class='<%# Eval("Notes").ToString() == "Pending" ? "fas fa-hourglass-half" : "fas fa-check-circle" %>'></i>
        <%# Eval("Notes") %>
    </span>
</td>

<td>
    <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("ExpenseItemId") %>')">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="studentModalLabel">Expense Management</h5>
                    <button type="button" class="close" data-dismiss="modal" onclick="refreshPage()" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <iframe id="studentIframe" src="ExpenseItemEdit.aspx" frameborder="0"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function confirmDelete(ExpenseId) {
            if (confirm("Are you sure you want to delete this Item?")) {
                window.location.href = 'ExpenseItemAdd.aspx?ExpenseItemId=' + ExpenseId + '&mode=delete';
            }
        }


        function openEditModal(ExpenseId) {
            document.getElementById('studentIframe').src = 'ExpenseItemEdit.aspx?ExpenseItemId=' + ExpenseId;
        }
