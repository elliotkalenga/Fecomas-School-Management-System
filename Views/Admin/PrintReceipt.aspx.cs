    ...
    public partial class PrintReceipt : System.Web.UI.Page
    {
        ...
        private void LoadFeesCollectionData(int feesCollectionId)
        {
            using (SqlConnection con = new SqlConnection(AppConnection.GetConnectionString()))
            {
                ...
                string query = @"
                    SELECT fc.FeesCollectionId, fc.ReferenceNo, fcs.Student, fcs.InvoiceId, fc.PaymentMethod,
                           fcs.FeesName + ' (' + CONVERT(VARCHAR, fcs.TotalFees) + ')' AS Fees,
                           fc.Amount AS ReceiptAmount, fcs.TotalCollected AS Cum_Collection,
                           fcs.Balance, fcs.InvoiceStatus  
                    FROM FeesCollectionSummary fcs
                    INNER JOIN FeesCollection fc ON fc.InvoiceId = fcs.InvoiceId
        ...
        {
            string query = @"
                SELECT FeesCollectionId, ReferenceNo, Student, InvoiceId, PaymentMethod, Fees,
                       ReceiptAmount, Cum_Collection, Balance, InvoiceStatus,
                       SchoolName, Logo, CreatedDate, Address, Term, CreatedBy
                FROM Receipt
            ...
            try
            {
                ...
                else
                {
                    ...
                    ReportDataSource reportDataSource = new ReportDataSource("ReceiptData", dataTable);
