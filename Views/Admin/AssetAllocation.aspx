<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="AssetAllocation.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.AssetAllocation" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Asset Allocation
                </button>
                <button type="button" class="btn btn-success mb-1" onclick="exportTableToCSV('Assets.csv')">
                    <i class="fas fa-file-csv"></i> Export CSV
                </button>
                <button type="button" class="btn btn-danger mb-1" onclick="exportTableToPDF('Assets.pdf')">
                    <i class="fas fa-file-pdf"></i> Export PDF
                </button>
            </div>
            <div class="d-flex flex-wrap gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search for Assets..." style="height: 30px;">
                        ...
                        <asp:Repeater ID="StudentsRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    ...
                                    <td><%# Eval("AssetDescription") %></td>
                                    <td><%# Eval("AssetCategory") %></td>
<td>
    <%# "<span style='color: " + (Eval("AllocatedStatus").ToString() == "Allocated" ? "#FFC107" : Eval("AllocatedStatus").ToString() == "Returned" ? "green" : "black") + "'>" + 
        "<i class='fa " + (Eval("AllocatedStatus").ToString() == "Allocated" ? "fa-share" : Eval("AllocatedStatus").ToString() == "Returned" ? "fa-check" : "") + "'></i> " + 
        Eval("AllocatedStatus") + 
        "</span>" %>
</td>                                    <td><%# $"{Eval("AssetHolder")}" %></td>
                                    <td><%# Eval("CreatedDate") != null ? Eval("CreatedDate", "{0:yyyy-MM-dd}") : "N/A" %></td>
                                    <td><%# Eval("ReturnedDate") != null ? Eval("ReturnedDate", "{0:yyyy-MM-dd}") : "N/A" %></td>                                    <td>
                                        <span class='<%# GetStatusClass(Eval("AssetStatus").ToString()) %>'>
                                            <i class='<%# GetStatusIcon(Eval("AssetStatus").ToString()) %>'></i>
                                            <%# Eval("AssetStatus") %>
                                        </span>
                                    </td>                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("AllocationId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button></td><td>
                                        <button type="button" class="btn btn-primary btn-sm" onclick="confirmDelete('<%# Eval("AllocationId") %>')">
                                            <i class="fas fa-reply"></i>Return Asset
                                        </button>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Assets Allocation Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="AssetAllocationAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog2" role="document" style="max-width: 400px;">
        <div class="modal-content">
            ...
            <div class="modal-body text-success" style="border: 2px solid green;">
                Asset Allocation deleted successfully!
            </div>
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to return this Asset?")) {
                window.location.href = 'AssetAllocationAdd.aspx?AllocationId=' + AllocationId + '&mode=delete';
            }
        }

        function openEditModal(AllocationId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'AssetAllocationAdd.aspx?AllocationId=' + AllocationId;
        }
