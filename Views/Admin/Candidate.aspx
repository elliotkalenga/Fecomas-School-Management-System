<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Candidate.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Candidate" %>
...
.page-link.active {
    ...
    max-width: 90%; /* Set the maximum width to 75% */
    max-height: 95%; /* Ensure the width is 75% */
}
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center" style="padding: 5px;">
                ...
                <div class="d-flex" style="margin: 0;">
                    <input type="text" id="searchInput" class="form-control mr-1" placeholder="Search for Candidate..." style="height: 30px;">
        ...
        <asp:Repeater ID="RecordsRepeater" runat="server">
            <ItemTemplate>
                <tr class="student-row">
                    ...
                    <td><%# Eval("Guardian") %></td>
                    <td>
                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("CandidateID") %>')">
                            <i class="fas fa-edit"></i> 
                        </button> </td> <td>
                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("CandidateID") %>')">
                            <i class="fas fa-trash"></i> 
                        </button></td> <td>
<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal2" onclick="openEditModal2('<%# Eval("CandidateID") %>')">
...
</div><div id="pagination"  class="pagination-container d-flex align-content-center"  ></div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="studentModalLabel2">Send Admission Letter</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <iframe id="studentIframe2" src="SendAdmissionLetter.aspx" frameborder="0"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="studentModal" tabindex="-1" role="dialog" aria-labelledby="studentModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="studentModalLabel">Candidate Registration</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <iframe id="studentIframe" src="CandidateAdd.aspx" frameborder="0"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to delete this Candidate?")) {
                window.location.href = 'CandidateAdd.aspx?CandidateID=' + studentID + '&mode=delete';
            }
        }

        function openEditModal(studentID, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'CandidateAdd.aspx?CandidateID=' + studentID;
        }

        function openEditModal2(studentID, mode) {
            var iframe = document.getElementById('studentIframe2');
            iframe.src = 'SendAdmissionLetter.aspx?CandidateID=' + studentID;
        }
