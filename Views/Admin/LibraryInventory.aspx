<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="LibraryInventory.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.LibraryInventory" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Book Into the Library
                </button>
                <button type="button" class="btn btn-success mb-1" onclick="exportTableToCSV('LibraryInventory.csv')">
                    <i class="fas fa-file-csv"></i> Export CSV
                </button>
                <button type="button" class="btn btn-danger mb-1" onclick="exportTableToPDF('LibraryInventory.pdf')">
                ...
                <asp:Repeater ID="RecordRepeater" runat="server">
                    <ItemTemplate>
                        <tr class="student-row">
                            ...
                            <td>
                                <%# Eval("BookStatus").ToString() == "Available" ? 
                                    "<i class='fas fa-thumbs-up text-success'></i> Available" : 
                                    "<i class='fas fa-times text-danger'></i> Not Available" 
                                %>
                            </td>

                            <td>
                                <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("InventoryId") %>')">
                                    <i class="fas fa-edit"></i> 
                                </button>
                                <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("InventoryId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Library Books Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="LibraryInventoryAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        function confirmDelete(InventoryId) {
            if (confirm("Are you sure you want to delete this Term?")) {
                window.location.href = 'LibraryInventoryAdd.aspx?InventoryId=' + InventoryId + '&mode=delete';
            }
        }

        function openEditModal(InventoryId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'LibraryInventoryAdd.aspx?InventoryId=' + InventoryId;
        }
