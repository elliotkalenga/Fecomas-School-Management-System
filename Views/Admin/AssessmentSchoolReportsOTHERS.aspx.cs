...
namespace SMSWEBAPP.Views.Admin
{
    public partial class AssessmentSchoolReportsOTHERS : System.Web.UI.Page
    {
        ...
        private void PopulateDropDownLists()
        {
            ...
            {
                string MSCEPerStudentQry = @"SELECT Distinct  StudentName+' '+ AssessmentTitle AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                            FROM MSCEOthers 
                            WHERE Schoolcode = @SchoolCode AND AssessmentTitle NOT LIKE '%END OF TERM%'";

                string JCEPerStudentQry = @"SELECT Distinct  StudentName+' '+ AssessmentTitle AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                            FROM JCEOthers
                            WHERE Schoolcode = @SchoolCode AND AssessmentTitle NOT LIKE '%END OF TERM%'";

                string PRIMARYPerStudentQry = @"SELECT Distinct  StudentName+' '+ AssessmentTitle AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                                FROM PRIOthers 
                            WHERE Schoolcode = @SchoolCode AND AssessmentTitle NOT LIKE '%END OF TERM%'";
                Con.Open();
                PopulateDropDownList(Con, MSCEPerStudentQry, ddlAssessmentStudent, "StudentName", "StudentNameId", "----Please Select Student----");
                PopulateDropDownList(Con, JCEPerStudentQry, ddlAssessment, "StudentName", "StudentNameId", "----Please Select Student----");
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName+' '+ AssessmentTitle = @StudentName) Order By Score Desc";
