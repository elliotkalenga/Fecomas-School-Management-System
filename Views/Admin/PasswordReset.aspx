<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="PasswordReset.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.PasswordReset" %>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            ...
            <div class="d-flex flex-wrap gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search for Users..." style="height: 30px;">
...
<asp:Repeater ID="RepeaterControl" runat="server">
    <ItemTemplate>
                                <tr>
                                    ...
                                    <td><%# Eval("LastName") %></td>
                                    <td><%# Eval("UserName") %></td>
                                    <td><%# Eval("RoleTitle") %></td>
                                    <td><%# Eval("UserStatus") %></td>
                                    <td>
<button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("UserId") %>')">
...
<div class="modal fade" id="studentModal" tabindex="-1" role="dialog" aria-labelledby="studentModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document" style="max-width: 80%; height: 80vh;">
        <div class="modal-content" style="height: 100%;">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Users Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" style="height: calc(100% - 60px); overflow-y: auto;">
                <iframe id="studentIframe" src="PasswordResetAdd.aspx" style="width: 100%; height: 100%; border: none;" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="successModal" tabindex="-1" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            var rows = $('#studentsTable tbody tr'); // Correct selector for table rows
            ...
            $('#searchInput').on('input', function () {
                ...
                rowsCount = $('#studentsTable tbody tr:visible').length;
                pageCount = Math.ceil(rowsCount / rowsPerPage);
                numbers.empty(); // Clear existing pagination
                for (var i = 1; i <= pageCount; i++) {
                    numbers.append('<a href="#" class="page-link" data-page="' + i + '">' + i + '</a>');
                }

                showPage(1); // Show first page after filtering
            });

            // Handling modal close event to refresh the page
        ...
        function confirmDelete(UserId) {
            if (confirm("Are you sure you want to delete this Score?")) {
                window.location.href = 'PasswordResetAdd.aspx?UserId=' + UserId + '&mode=delete';
            }
        }

        function openEditModal(UserId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'PasswordResetAdd.aspx?UserId=' + UserId;
        }
