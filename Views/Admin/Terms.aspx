<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Terms.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Terms" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Term
                </button>
                        ...
                        <asp:Repeater ID="RecordRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    <td><%# Eval("Term") %></td>
                                    <td><%# Eval("StartDateString") %></td>
                                    <td><%# Eval("EndDateString") %></td>
                                    <td><%# Eval("Status") %></td>
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("TermId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("TermId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Term Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="TermsAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to delete this Term?")) {
                window.location.href = 'TermsAdd.aspx?TermId=' + TermId + '&mode=delete';
            }
        }

        function openEditModal(TermId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'TermsAdd.aspx?TermId=' + TermId;
        }
