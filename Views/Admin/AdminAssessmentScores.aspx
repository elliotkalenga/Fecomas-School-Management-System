<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="AdminAssessmentScores.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.AdminAssessmentScores" %>
...
<div class="container">
    <div class="row text-center">
        ...
        <div class="col-md-4 mb-3">
            <button type="button" class="btn btn-dark-blue w-100" data-toggle="modal" data-target="#studentModal">
                <i class="fas fa-user-plus"></i> Add New Score
            </button>
        </div>
        <div class="col-md-4 mb-3">
        ...
        <div class="col-md-4 mb-3">
            <button type="button" class="btn btn-primary w-100" onclick="window.location.href='Assessment.aspx'">
                <i class="fas fa-list"></i> Go back to Assessments List
            </button>
        </div>
    </div>
</div>
            <div class="d-flex flex-wrap align-items-center gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search for Scores..." style="height: 30px;">
                <!-- Optional: Add a search button if needed -->
                <!-- <button type="button" class="btn btn-info" onclick="searchTable()" style="height: 30px;">
                ...
                <table id="studentsTable" class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        <asp:Repeater ID="ScoresRepeater" runat="server">
                            <ItemTemplate>
                                <tr>
                                    <td><%# $"{Eval("ScoreRank")} Out of {Eval("ScoreCount")}" %></td>
                                    <td><%# Eval("Student") %></td>
                                    <td><%# Eval("AssessmentTitle") %></td>
                                    <td><%# Eval("ExamScore") %></td>
                                    <td><%# Eval("Grade") %></td>
                                    <td><%# Eval("GradeDescription") %></td>
                                    <td><%# Eval("Remark") %></td>
                                    <td><%# Eval("SubjectTeacher") %></td>
                                    <td><%# $"{Eval("SubjectName")}-{Eval("Term")}" %></td>
                                    <td><%# Eval("Contribution") %></td>
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("ScoreId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("ScoreId") %>')">
    ...
    <div class="modal-dialog modal-lg" role="document" style="max-width: 90vw; width: 90vw; height: 90vh;">
        <div class="modal-content" style="height: 100%;">
            ...
            <div class="modal-body" style="height: calc(100% - 50px); padding: 0;">
                <iframe id="reportIframe" src="AssessmentScoresReports.aspx" frameborder="0" style="width: 100%; height: 100%; border: none;"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="studentModal" tabindex="-1" role="dialog" aria-labelledby="studentModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Scores Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="AdminAssessmentScoresAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function openEditModal(ScoreId) {
        var iframe = document.getElementById('studentIframe');
        iframe.src = 'AssessmentScoreAdd.aspx?mode=edit&ScoreId=' + encodeURIComponent(ScoreId);
        $('#studentModal').modal('show');
    }
