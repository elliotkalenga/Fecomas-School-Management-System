<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="FeesStructure.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.FeesStructure" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Fees Structure
                </button>
            ...
            <div class="d-flex flex-wrap gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search Fees Structure..." style="height: 30px;">
                        ...
                        <asp:Repeater ID="RecordsRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    ...
                                    <td><%# Eval("Amount") %></td>
                                    <td><%# Eval("Status") %></td>
                                    <td><%# Eval("CreatedDateString") %></td> <!-- Fixed missing closing tag -->
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("FeesId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("FeesId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Fees Structure Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="FeesStructureAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="successModal" tabindex="-1" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            function confirmDelete(FeesId) {
            if (confirm("Are you sure you want to delete this Exam?")) {
                window.location.href = 'FeesStructureAdd.aspx?FeesId=' + FeesId + '&mode=delete';
            }
        }

        function openEditModal(FeesId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'FeesStructureAdd.aspx?FeesId=' + FeesId;
        }
