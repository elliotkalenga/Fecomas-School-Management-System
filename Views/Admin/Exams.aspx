<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Exams.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Exams" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Exam
                </button>
                <button type="button" class="btn btn-success mb-1" onclick="exportTableToCSV('Exams.csv')">
                    <i class="fas fa-file-csv"></i> Export CSV
                </button>
                <button type="button" class="btn btn-danger mb-1" onclick="exportTableToPDF('Exams.pdf')">
                    <i class="fas fa-file-pdf"></i> Export PDF
                </button>
            </div>
            <div class="d-flex flex-wrap gap-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search for Exams..." style="height: 30px;">
                        ...
                        <asp:Repeater ID="StudentsRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    ...
                                    <td><%# Eval("ExamType") %></td>
                                    <td><%# Eval("Term") %></td>
                                    <td><%# Eval("ExamStatus") %></td>
                                    <td><%# Eval("ExamWeight") %></td> <!-- Fixed missing closing tag -->
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("ExamId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("ExamId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Exam Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="ExamAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to delete this Exam?")) {
                window.location.href = 'ExamAdd.aspx?ExamId=' + ExamId + '&mode=delete';
            }
        }

        function openEditModal(ExamId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'ExamAdd.aspx?ExamId=' + ExamId;
        }
