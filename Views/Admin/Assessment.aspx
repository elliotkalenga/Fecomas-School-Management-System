<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Assessment.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Assessment" %>
...
<div class="card-header d-flex justify-content-between align-items-center p-2">
    <div class="d-flex flex-wrap align-items-center" style="gap: 20px;">
        <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
            <i class="fas fa-user-plus"></i> Add New Assessment
        </button>
        <button type="button" class="btn btn-dark-blue mb-1" onclick="window.location.href='AssessmentScore.aspx'">
            <i class="fas fa-camera"></i> Capture Scores
        </button>
        <button type="button" class="btn btn-success mb-1" onclick="exportTableToExcel('Assessments.xlsx')">
            <i class="fas fa-file-excel"></i> Export Excel
        </button>
    </div>
    <div class="d-flex flex-wrap align-items-center gap-2">
        <input type="text" id="searchInput" class="form-control" placeholder="Search for Assessment..." style="height: 30px;">
                        ...
                        <asp:Repeater ID="StudentsRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    ...
                                    <td><%# Eval("Status") %></td>
                                    <td><%# Eval("Contribution") + "%" %></td>                                    <!-- Fixed missing closing tag -->
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("AssessmentId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("AssessmentId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Assessment Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="AssessmentAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="successModal" tabindex="-1" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        $(document).ready(function () {
            var rowsPerPage = 40; // Number of records per page
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to delete this Assessment?")) {
                window.location.href = 'AssessmentAdd.aspx?AssessmentId=' + AssessmentId + '&mode=delete';
            }
        }

        function openEditModal(AssessmentId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'AssessmentAdd.aspx?AssessmentId=' + AssessmentId;
        }
