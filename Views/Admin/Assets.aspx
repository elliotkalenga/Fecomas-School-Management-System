<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="Assets.aspx.cs" Inherits="SMSWEBAPP.Views.Admin.Assets" %>
        ...
        <div class="card-header d-flex justify-content-between align-items-center p-2">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="button" class="btn btn-dark-blue mb-1" data-toggle="modal" data-target="#studentModal">
                    <i class="fas fa-user-plus"></i> Add New Asset
                </button>
                ...
                <table id="studentsTable" class="table table-striped table-bordered" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>Category</th>
                            <th>Initial Value</th>
                            <th>Lifespan In (Yrs)</th>
                            <th>Lifespan In (dys)</th>
                            <th>UsedSpan In (dys)</th>
                            <th>Remaining Span In (dys)</th>
                        ...
                        <asp:Repeater ID="StudentsRepeater" runat="server">
                            <ItemTemplate>
                                <tr class="student-row">
                                    ...
                                    <td><%# $"K{Eval("PurchaseCost")}" %></td>
                                    <td><%# $"{Eval("LifeSpan")} Years" %></td>
                                    <td><%# $"{Eval("LifespanInDays")} Days" %></td>    
                                    <td><%# $"{Eval("UsedSpan")} Days" %></td>
                                    <td><%# $"{Eval("RemainingSpan")} Days" %></td>
                                    <td><%# $"{Eval("UsagePercent")}% Used" %></td>
                                    <td><%# $"K{Eval("AssetValue")}" %></td>
                                    <td>
                                        <%# "<span style='color: " + (Eval("AllocatedStatus").ToString() == "Allocated" ? "#FFC107" : Eval("AllocatedStatus").ToString() == "Returned" ? "#8BC34A" : "#2196F3") + "'>" + 
                                            "<i class='fa " + (Eval("AllocatedStatus").ToString() == "Allocated" ? "fa-share" : Eval("AllocatedStatus").ToString() == "Returned" ? "fa-check" : "fa-box") + "'></i> " + 
                                        ...
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#studentModal" onclick="openEditModal('<%# Eval("AssetId") %>')">
                                            <i class="fas fa-edit"></i> 
                                        </button></td><td>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('<%# Eval("AssetId") %>')">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentModalLabel">Assets Management</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="studentIframe" src="AssetsAdd.aspx" frameborder="0"></iframe>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        var urlParams = new URLSearchParams(window.location.search);
            ...
            if (confirm("Are you sure you want to delete this Asset?")) {
                window.location.href = 'AssetsAdd.aspx?AssetId=' + AssetId + '&mode=delete';
            }
        }

        function openEditModal(AssetId, mode) {
            var iframe = document.getElementById('studentIframe');
            iframe.src = 'AssetsAdd.aspx?AssetId=' + AssetId;
        }
