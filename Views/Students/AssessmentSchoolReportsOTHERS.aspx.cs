...
{
    public partial class AssessmentSchoolReportsOTHERS : System.Web.UI.Page
    {
        ...
        private void PopulateDropDownLists()
        {
            ...
            {
                string MSCEPerStudentQry = @"SELECT Distinct StudentNo,TermId, StudentName AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                            FROM MSCEOthers 
                            WHERE Schoolcode = @SchoolCode AND AssessmentTitle NOT LIKE '%END OF TERM%' and StudentNo=@StudentNo and releasestatus ='Released' Order by TermId Desc";

                string JCEPerStudentQry = @"SELECT Distinct StudentNo,TermId, StudentName AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                            FROM JCEOthers
                            WHERE Schoolcode = @SchoolCode AND AssessmentTitle NOT LIKE '%END OF TERM%' and StudentNo=@StudentNo and releasestatus ='Released' Order by TermId Desc";

                string PRIMARYPerStudentQry = @"SELECT Distinct StudentNo,TermId, StudentName  AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                                FROM PRIOthers 
                            WHERE Schoolcode = @SchoolCode AND AssessmentTitle NOT LIKE '%END OF TERM%' and StudentNo=@StudentNo and releasestatus ='Released' Order by TermId Desc";
                Con.Open();
                PopulateDropDownList(Con, MSCEPerStudentQry, ddlAssessmentStudent, "StudentName", "StudentNameId", "----Please Select Exams----");
                PopulateDropDownList(Con, JCEPerStudentQry, ddlAssessment, "StudentName", "StudentNameId", "----Please Select Exams----");
                PopulateDropDownList(Con, PRIMARYPerStudentQry, ddlAggregate, "StudentName", "StudentNameId", "----Please Select Exams ---");
            }
        }
      ...
      ,[Term]
      ,[TermId]
      ,CAST(FLOOR([Score]) AS INT) AS Score
      ,[Grade]
      ,[GradeDescription]
      ,[Remark]
      ,[SubjectTeacher]
      ...
      ,[Position]
      ,[ClassCount]
  FROM [FECOMASDESK].[dbo].[PRIOthers]
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName = @StudentName) Order By Score Desc";
            ...
            using (SqlConnection Con = new SqlConnection(AppConnection.GetConnectionString()))
            {
                using (SqlCommand command = new SqlCommand(query, Con))
                {
                    ...
                    command.Parameters.AddWithValue("@StudentName", txtAggregate.Text.ToString());
            ...
            else
            {
                ...
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/PRIOthers.rdlc");

                // Enable external images
                ReportViewer1.LocalReport.EnableExternalImages = true;
                // Add the data source to the report
                ReportDataSource reportDataSource = new ReportDataSource("PRIOthers", dataTable);
                ...
                ReportViewer1.LocalReport.DisplayName = (studentName + assessment).Replace(" ", string.Empty);


                string studentfilename = (studentName + assessment + classname)
      ...
      ,[Position]
      ,[ClassCount]
  FROM [FECOMASDESK].[dbo].[JCEOthers]
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName = @StudentName) Order By Score Desc";
            ...
            using (SqlConnection Con = new SqlConnection(AppConnection.GetConnectionString()))
            {
                ...
                {
                    command.Parameters.AddWithValue("@SchoolCode", Session["SchoolCode"]);
                    command.Parameters.AddWithValue("@StudentName", txtAssessment.Text.ToString());
                    command.CommandTimeout = 130;
                    using (SqlDataAdapter dataAdapter = new SqlDataAdapter(command))
                    {
                        dataAdapter.Fill(dataTable);
            ...
            else
            {
                ...
                // Set the ReportViewer properties
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/JCEOthers.rdlc");

                // Enable external images
                ReportViewer1.LocalReport.EnableExternalImages = true;
                // Add the data source to the report
                ReportDataSource reportDataSource = new ReportDataSource("JCEOthers", dataTable);
                ReportViewer1.LocalReport.DataSources.Clear();
                ReportViewer1.LocalReport.DataSources.Add(reportDataSource);
      ...
      ,[Position]
      ,[ClassCount]
  FROM [FECOMASDESK].[dbo].[MSCEOthers]
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName = @StudentName) Order By Score Desc";
            ...
            else
            {
                ...
                // Set the ReportViewer properties
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/MSCEOthers.rdlc");
