...
{
    public partial class AssessmentScoresEOTReports : System.Web.UI.Page
    {
        ...
        private void PopulateDropDownLists()
        {
            using (SqlConnection Con = new SqlConnection(AppConnection.GetConnectionString()))
            {
                string MSCEPerStudentQry = @"SELECT Distinct StudentNo,TermId,  StudentName AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                            FROM MSCEEndofterm 
                            WHERE Schoolcode = @SchoolCode and StudentNo=@StudentNo and releasestatus ='Released' Order by TermId Desc";
                ...
                string PRIMARYPerStudentQry = @"SELECT Distinct StudentNo,TermId, StudentName  AS StudentNameId, StudentName+' '+ AssessmentTitle as StudentName
                            ...
                            WHERE Schoolcode = @SchoolCode and StudentNo=@StudentNo and releasestatus ='Released' Order by TermId Desc";
                Con.Open();
                PopulateDropDownList(Con, MSCEPerStudentQry, ddlAssessmentStudent, "StudentName", "StudentNameId", "----Please Select Term----");
                PopulateDropDownList(Con, JCEPerStudentQry, ddlAssessment, "StudentName", "StudentNameId", "----Please Select Term----");
                PopulateDropDownList(Con, PRIMARYPerStudentQry, ddlAggregate, "StudentName", "StudentNameId", "----Please Select Term ---");
            }
        }
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName = @StudentName) Order By Score Desc";
            ...
            else
            {
                ...
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/PRIEndoftermReport.rdlc");

                // Enable external images
                ReportViewer1.LocalReport.EnableExternalImages = true;
                // Add the data source to the report
                ReportDataSource reportDataSource = new ReportDataSource("PRIEndoftermReport", dataTable);
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName = @StudentName) Order By Score Desc";
            ...
            else
            {
                ...
                // Set the ReportViewer properties
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/JCEEndoftermReport.rdlc");
...
WHERE  (SchoolCode = @SchoolCode) AND (StudentName = @StudentName) Order By Score Desc";
            ...
            else
            {
                ...
                // Set the ReportViewer properties
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/MSCEEndofterm.rdlc");
